...
Когда что: существенная переработка запроса (добавить поля, соединения, переписать пакет) -> получи запрос через `unica.dcs.info` с `Mode=query`, `Name=<набор>`, `"Raw": true`; отредактируй текст из ответа и передай его в `set-query` как `Value`. `Raw` отдаёт запрос целиком без декораций, поэтому передача текста точна, включая многопакетные запросы. Точечная замена идентификатора или подстроки не требует выгрузки: используй `patch-query`.
