...
| `ValueStorage` | `v8:ValueStorage` (только как единственный тип) |

Для профиля 8.3.27 параметры всегда целые: `String(length)` принимает
`0..=1024`, `Number(digits,fraction[,nonneg])` — `digits` в `0..=38` и
`fraction` в `0..=digits`. В составном типе нельзя дважды задавать один
платформенный тип (`String(50) + String(20)`, `Date + DateTime` и т. п.);
`ValueStorage` нельзя объединять с другими типами.

Содержимое `v8:TypeDescription` группируется по XSD 8.3.27: конкретные
`Type`, затем `TypeSet` и квалификаторы Number → String → Date. Порядок
конфигурационных `Type` внутри группы платформа определяет по `xr:TypeId` из
полного индекса `GeneratedType`; автономный компилятор без этого индекса
сохраняет входной порядок и не имитирует заведомо неверную сортировку по имени.

Имя конфигурационного типа содержит ровно один разделитель `.` и обе части
должны образовывать допустимый XML NCName; пробелы, двоеточия и дополнительная
точка отклоняются до записи.
...
Произвольные bare-типы и raw QName (`xs:string`, `v8:UUID` и т. п.) этот
фиксированный DSL не принимает: они не подменяют перечисленные выше типы и не
обходят их qualifiers/ограничения.
...
Для `fillChecking` профиль 8.3.27 принимает только `DontCheck` и
`ShowError`. Значение `ShowWarning` в XDTO 8.3.27 отсутствует и отклоняется до
записи XML.
...
| Поле JSON | Умолчание | XML элемент |
|-----------|----------|-------------|
| `source` | `[]` | Source: массив DSL-типов, объединяемый в один `v8:TypeDescription`; exact-platform proof смешанного состава пока ожидает финальный gate |
| `event` | `BeforeWrite` | Event |
| `handler` | `""` | Handler (авто-префикс `CommonModule.`) |

Без ChildObjects и модулей.

На уровне сериализатора все элементы `source` образуют один
`v8:TypeDescription`: сначала выводятся все `Type`, затем `TypeSet` и
квалификаторы. Дубликаты платформенных типов и несовместимые комбинации
(включая `ValueStorage` с другим типом) проверяются по всему массиву, а не по
каждому элементу отдельно.

Обязательный корпус теперь содержит смешанный source из конфигурационного типа
и `String(10)`, но его точный round-trip через `ibcmd 8.3.27.2074` ещё не прошёл
финальный gate. До прохождения gate это контракт сериализатора и ожидаемая
XSD-форма, а не доказанная платформенная гарантия.
...
автоматически подставлять или мигрировать ссылку инструмент не будет.
